...
python blast_xml_parser.py in_file out_file out_format
"""
...
elif out_fmt == "ext" or out_fmt == "ext+":
    extended = True
else:
    stop_err("Format argument should be std (12 column) or ext (extended 24 columns) or ext+ (extended 26 columns)")
    ...
    if event == "end" and elem.tag == "Iteration":
        ...
        for hit in elem.findall("Iteration_hits/Hit"):
            ...
            for hsp in hit.findall("Hit_hsps/Hsp"):
                ...
                qstart = hsp.findtext("Hsp_query-from")
                qend = hsp.findtext("Hsp_query-to")
                values = [qseqid,
                          sseqid,
                          pident,
                          length, #hsp.findtext("Hsp_align-len")
                ...
                if extended:
                    ...
                    if extended_plus:
                        pcov = "%0.2f" % (float(int(qend) - int(qstart) + 1)/qlen * 100)
                        sallseqdescr = ";".join(name.split(None,1)[1] for name in hit_def.split(">"))
                        values.extend([pcov, sallseqdescr])
